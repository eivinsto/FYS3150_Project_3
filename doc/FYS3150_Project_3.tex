...
\begin{align*}
U_\text{tot} = \sum\limits_i \sum\limits_{j\neq i} U(\vec{r}_{ij},t) \, ,
\end{align*}

where $U$ here is the potential energy experienced by body $i$ because of body $j$. This also means that we can write the force experienced by body $i$ from body $j$ as:
...
\begin{align*}
\frac{d}{dt}U_\text{tot} &= \sum\limits_{i} \sum\limits_{j\neq i} \frac{d}{dt} U (\vec{r}_{ij}, t ) \, , \\
...
&= \frac{\partial}{\partial t} U_\text{tot} + \sum\limits_{i} \sum\limits_{j\neq i} \gradient_{ij} U (\vec{r}_{ij}, t ) \cdot \frac{d\vec{r}_{ij}}{dt} \\
...
&= - \sum\limits_j \sum\limits_{i \neq j} \frac{d\vec{r}_j}{dt} \vec{F}_{ij} \\
&= - \sum\limits_i \sum\limits_{j \neq i} \frac{d\vec{r}_j}{dt} \cdot \vec{F}_{ij}
\end{align*}

where we switched the indexation around in the second to last equality, and rewrote the summation in the last equality.
...
\end{align*}
as the escape velocity of a planet startin at a distance of \(1\) AU from the Sun.
