...
We then find the acceleration of body \(i\) by dividing the net force by \(m_{a}\) resulting in
\begin{align*}
	\vec{a}_{i}(t, \vec{r}_{i}(t)) = \frac{\vec{F}_{i}(t, \vec{r}_{i}(t))}{m_{i}} = -G\frac{\vec{r}_{i}(t)-\vec{r}_{j}(t)}{|\vec{r}_{i}(t)-\vec{r}_{j}(t)|^{3}},
\end{align*}
which can be written as the differential equation
\begin{align*}
 	\frac{d\vec{v}_{i}(t)}{dt} = -G\frac{\vec{r}_{i}(t)-\vec{r}_{j}(t)}{|\vec{r}_{i}(t)-\vec{r}_{j}(t)|^{3}}. \numberthis \label{eq:gendv}
\end{align*}
...
Solving \eqref{eq:gendv} and \eqref{eq:gendr} for every body \(i=1, ..., n\), allowes us to find the internal motion of a given n-body system with only gravitational interaction.
