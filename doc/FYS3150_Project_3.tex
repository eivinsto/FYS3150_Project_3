...
To solve our equations of motion we require methods for numerical integration. To derive our first method, we consider the genereic function \(f\) that depends on a variable \(t\) such that
\begin{align*}
...
\begin{align*}
	\int_{t_{0}}^{t_{1}} f(t) dt = \int_{t_{0}}^{t_{1}} g(t, f(t)) dt.
\end{align*}
Applying the fundemental theorem of calculus, we get that
\begin{align*}
	f(t_{1}) - f(t_{1}) = \int_{t_{0}}^{t_{1}} g(t, f(t)) dt, \\
...
\end{align*}
This method of numerical integration is known as Euler's method, henceforth refered to as the forward Euler method.
